...
export default function CommentsPanel({
  ...
  return (
    <div className="fixed right-0 top-0 h-full w-full sm:w-96 md:w-96 lg:w-80 bg-white shadow-xl flex flex-col border-l border-gray-200 z-50 animate-in slide-in-from-right">
      {/* Header */}
      ...
      {/* Highlight Preview */}
      <div className="p-3 md:p-4 bg-slate-50 border-b border-gray-200">
        <p className="text-xs text-gray-500 mb-2">Highlighted text</p>
        <blockquote className="rounded-md border-l-4 border-blue-400 bg-white px-2 md:px-3 py-2 text-xs md:text-sm text-gray-800 leading-relaxed">
          {highlight.text}
        </blockquote>
        <p className="text-xs text-gray-500 mt-2">By {highlight.username}</p>
      </div>

      {/* Comments List */}
      <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-2 md:space-y-3">
        {highlightComments.length === 0 ? (
          <div className="text-center text-gray-500 text-xs md:text-sm py-8">
            <MessageCircle size={32} className="mx-auto mb-2 opacity-20" />
            <p>No comments yet</p>
            <p className="text-xs">Be the first to add!</p>
          </div>
        ) : (
          highlightComments.map((comment, idx) => (
            <div
              key={comment._id || idx}
              className="bg-gray-50 p-2 md:p-3 rounded-lg hover:bg-gray-100 transition"
            >
      ...
      <form
        ...
        className="sticky bottom-0 p-3 md:p-4 border-t border-gray-200 bg-gray-50"
      >
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-2 md:px-3 py-2 text-xs md:text-sm text-gray-900 placeholder:text-gray-500 caret-gray-900 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
          />
          <button
            type="submit"
