...
try {
  ...
  const root = await fetch('https://ai-learning-platform-c2jg.onrender.com/api/ai');
  console.log('GET /api/ai status', root.status);
  const authRoot = await fetch('https://ai-learning-platform-c2jg.onrender.com/api/auth');
  console.log('GET /api/auth status', authRoot.status);

  console.log('\nNow GET /api/flashcards (protected)');
  const f = await fetch('https://ai-learning-platform-c2jg.onrender.com/api/flashcards', { headers: { 'Authorization': `Bearer ${token}` } });
  console.log('/api/flashcards status', f.status);
  console.log('/api/flashcards body', await f.text());

  const res = await fetch('https://ai-learning-platform-c2jg.onrender.com/api/ai/generate-flashcards', {
    method: 'POST',
    headers: {
  ...
  const hist = await fetch('https://ai-learning-platform-c2jg.onrender.com/api/ai/chat-history/696664e5f67efd758e2b6fdc', {
    method: 'GET',
  ...
  const chatResp = await fetch('https://ai-learning-platform-c2jg.onrender.com/api/ai/chat', {
    method: 'POST',
